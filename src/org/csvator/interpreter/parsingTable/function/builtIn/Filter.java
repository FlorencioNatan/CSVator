...
public class Filter implements FunctionValueInterface {
	...
	public Environment createLocalEnvironment(LinkedList<ValueInterface> values, Environment father) throws TypeMismatchException {
		if (values.size() != 2) {
			throw new InvalidNumberOfParametersException("The function filter expects 2 parameters, but " + values.size() +  " are found.");
		}
		...
		local.putValue("filterFunction", sortFunction);
		return local;
	}
	...
	public ValueInterface apply(Environment env) {
		...
		FunctionValueInterface filterFunction = (FunctionValueInterface) env.getValueOf("filterFunction");
