...
public abstract class EqualityOperator implements OperatorInterface {
	...
	public ValueInterface apply(ValueInterface lho, ValueInterface rho, Environment env) {
		...
		differentClassComparsion(lho, rho, env);
		intComparsion(lho, rho, env);
		stringComparsion(lho, rho, env);
		datetimeComparsion(lho, rho, env);
		booleanComparsion(lho, rho, env);
		nullComparsion(lho, rho, env);

		return comparsionResult;
	}

	private void booleanComparsion(ValueInterface lho, ValueInterface rho, Environment env) {
		if (lho.getType() == BoolTypeValue.getInstance()) {
			BooleanValue booleanLho = this.castToBooleanValue(lho);
			BooleanValue booleanRho = this.castToBooleanValue(rho);
			boolean result = this.operationOnBoolean(booleanLho.getBooleanValue(), booleanRho.getBooleanValue());
	...
	private void differentClassComparsion(ValueInterface lho, ValueInterface rho, Environment env) {
		if (!lho.getClass().equals(rho.getClass())) {
			boolean result = this.operationOnDifferentClasses(lho, rho);
			comparsionResult = this.createResult(lho, rho, result, env);
		}
	}
	...
	private void nullComparsion(ValueInterface lho, ValueInterface rho, Environment env) {
		if (lho.getType() == VoidTypeValue.getInstance()) {
			boolean result = this.operationOnNull(lho, rho);
			comparsionResult = this.createResult(lho, rho, result, env);
		}
	}
	...
	abstract protected boolean operationOnBoolean(boolean lho, boolean rho);
	abstract protected boolean operationOnNull(ValueInterface lho, ValueInterface rho);
	abstract protected boolean operationOnDifferentClasses(ValueInterface lho, ValueInterface rho);
	...
	private BooleanValue castToBooleanValue(ValueInterface value) {
